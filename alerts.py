...
TELEGRAM_GRADES = {"S", "A"}
PUSHOVER_GRADES = {"S", "A", "B", "C"}
YAHOO_GRADES = {g.strip().upper() for g in os.getenv("YAHOO_GRADES", "S,A").split(",") if g.strip()}
...
def fire_alerts(signals: List[Dict[str, Any]], quiet: bool = False, sprint_mode: bool = False) -> None:
    logger.info("fire_alerts called count=%s quiet=%s sprint=%s", len(signals), quiet, sprint_mode)
    for signal in signals:
        grade = str(signal.get("grade", "")).upper()
        pair = signal.get("pair", "?")
        bias = signal.get("bias", "?")
        engine = signal.get("engine", "?")
        conv = signal.get("final_conviction", "?")
        remi = signal.get("remi_status", "CLEAN")
        msg = _format_signal(signal, quiet=quiet)
        if grade in TELEGRAM_GRADES and not quiet:
            _send_telegram(msg)
        elif grade in TELEGRAM_GRADES and quiet:
            logger.info("[TELEGRAM suppressed quiet] %s %s %s conv=%s", pair, bias, engine, conv)
        ...
        logger.info("[ALERT] %s %s %s grade=%s conv=%s remi=%s", pair, bias, engine, grade, conv, remi)
        logger.info("ALERT_ROUTE pair=%s grade=%s quiet=%s telegram=%s pushover=%s yahoo=%s", pair, grade, quiet, grade in TELEGRAM_GRADES and not quiet, grade in PUSHOVER_GRADES, grade in YAHOO_GRADES)
    push_to_cf()
